...
P="plugins/yingu-music-service"
mkdir -p "$P/www" "$P/scripts" "$P/src/android/res/drawable" "$P/src/android/res/xml"

# package.json
cat > "$P/package.json" << 'EOF'
{"name":"yingu-music-service","version":"1.0.2","description":"音轨音乐服务插件","cordova":{"id":"yingu-music-service","platforms":["android"]},"author":"音轨","license":"Apache-2.0"}
EOF

# plugin.xml
cat > "$P/plugin.xml" << 'EOF'
...
# 通知栏图标
cat > "$P/src/android/res/drawable/ic_music_note.xml" << 'EOF'
...
cat > "$P/src/android/res/drawable/ic_play.xml" << 'EOF'
<vector xmlns:android="http://schemas.android.com/apk/res/android" android:width="24dp" android:height="24dp" android:viewportWidth="24" android:viewportHeight="24" android:tint="#FFFFFF"><path android:fillColor="#FFFFFF" android:pathData="M8,5v14l11,-7z"/></vector>
EOF

cat > "$P/src/android/res/drawable/ic_pause.xml" << 'EOF'
<vector xmlns:android="http://schemas.android.com/apk/res/android" android:width="24dp" android:height="24dp" android:viewportWidth="24" android:viewportHeight="24" android:tint="#FFFFFF"><path android:fillColor="#FFFFFF" android:pathData="M6,5h4v14H6zM14,5h4v14h-4z"/></vector>
EOF

cat > "$P/src/android/res/drawable/ic_skip_prev.xml" << 'EOF'
<vector xmlns:android="http://schemas.android.com/apk/res/android" android:width="24dp" android:height="24dp" android:viewportWidth="24" android:viewportHeight="24" android:tint="#FFFFFF"><path android:fillColor="#FFFFFF" android:pathData="M6,6h2v12H6zM9.5,12l8.5,6V6z"/></vector>
EOF

cat > "$P/src/android/res/drawable/ic_skip_next.xml" << 'EOF'
<vector xmlns:android="http://schemas.android.com/apk/res/android" android:width="24dp" android:height="24dp" android:viewportWidth="24" android:viewportHeight="24" android:tint="#FFFFFF"><path android:fillColor="#FFFFFF" android:pathData="M16,6h2v12h-2zM6,18l8.5,-6L6,6z"/></vector>
EOF

# 网络安全配置
cat > "$P/src/android/res/xml/network_security_config.xml" << 'EOF'
<?xml version="1.0" encoding="utf-8"?>
...
# YinguMusicPlugin.java
cat > "$P/src/android/YinguMusicPlugin.java" << 'JAVAEOF'
...
# MusicService.java
cat > "$P/src/android/MusicService.java" << 'JAVAEOF'
package com.yingu.player;
...
# generate-icons.js
cat > "$P/scripts/generate-icons.js" << 'JSEOF'
const fs = require('fs');
...
module.exports = function(context) {
  ...
  if (!resDir) {
    ...
    for (const c of candidates) {
      if (fs.existsSync(path.dirname(c))) { resDir = c; break; }
    }
  }
  if (!resDir) {
...
# modify_mainactivity.js
cat > "$P/scripts/modify_mainactivity.js" << 'JSEOF'
const fs = require('fs');
const path = require('path');
...
echo "[音轨] 插件文件创建完成"
ls -la "$P/plugin.xml" "$P/package.json" "$P/www/YinguMusic.js" "$P/src/android/MusicService.java" "$P/src/android/YinguMusicPlugin.java" "$P/scripts/generate-icons.js" "$P/scripts/modify_mainactivity.js"